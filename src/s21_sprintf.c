...
char* print_number(char* str, flag_t* flags, char type, va_list* input) {
  ...
  if (type == 'f') {
    ftostr(buffer, va_arg(*input, double), flags);
  } else if (type == 'u') {
    ...
    uitostr(buffer, input_data, flags);
  } else {
    ...
    itostr(buffer, input_data, flags);
  }
...
void ftostr(char* str, double num, flag_t* flags) {
  ...
  // count total number of digits in num converting it to long int
